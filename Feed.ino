...
void Feed_Ext_Front()
{  
  if (TENSION_Z > 0){ ///если натяг больше нуля
   ...
   {  
      if (cycle_flag == false && Pass_Nr <= Pass_Total)
      {
         ...
         if (Infeed_Value == 0)
         {
            ...
            else              Limit_Pos_Left = (Limit_Pos_Right);
         }
         else
         {
            if (Pass_Nr == 1) Limit_Pos_Left = (Null_Z_Pos + Infeed_Value);
            else              Limit_Pos_Left = (Limit_Pos_Right + Infeed_Value);
         }
         }
         else {
         if (Infeed_Value == 0)
      ...
      else if (cycle_flag == false && Pass_Nr > Pass_Total)
      {
         ...
         a_flag = false;
         b_flag = false; 
         Limit_Pos_Left = Limit_Pos_Right;
         Feed_Left(a_flag, b_flag);
         }
         else {
         a_flag = false;
