  ...
  if (!button_up_flag)
  {
     switch (Mode)
     {
        ...
        if (SelectMenu == 1 && !Joy_Z_flag && !Joy_X_flag)
        {
         ...
         if (!Button_Rapid){
         if         (Ap < 9900)  {Ap = Ap + 100; Beep();}}
         else { if  (Ap < 9900)  {Ap = Ap + 2;   Beep();}}}
         }
...
void Key_Left_Pressed()
{
   if (!button_left_flag)
   {
      switch (Mode)
      {
         ...
         Beep();
//         if (flag_Scale_x1 == ON){
//         for (int i = 0; i < LINEIKA ; i ++)
//         { MX_pos ++ ;}}else if {
//         for (int i = 0; i < LINEIKA*BT_X; i ++)  
//         { MX_pos ++ ;}} 
//          Beep();
         }         
         Print();
...
void Switch_Hand_Axis_No()
{
   ...
   hand_X = OFF;
   flag_hand_No = ON;
   if (flag_hand_No == ON && Mode == Mode_Feed && Scale == HC_SCALE_10){ BeepBeep();}
